...
const Schedule = () => {
  ...
  const handleScheduleSave = async () => {
    ...
    try {
      setLoading(true);
      await AxiosInstances.post("/availability", payload);
  ...
  const dateCellRender = (value) => {
    ...
    if (unavailableDates.includes(currentDate)) {
      return (
        <div className="rounded bg-red-500 p-1 text-center text-[10px] text-white sm:p-2 sm:text-xs">
          Unavailable
        </div>
      );
    }
      ...
      return (
        <div className="space-y-1">
          {bookedOnThisDay.map((slot, index) => (
            <div
              key={index}
              className="rounded bg-green-200 p-1 text-[10px] sm:text-xs"
            >
              {slot.slot}
            </div>
          ))}
  ...
  return (
    <Dashboard>
      <div className="px-4 py-4 sm:px-6 md:px-8">
        ...
        <div className="overflow-x-auto rounded-lg bg-white p-2 shadow-sm sm:p-4">
          <Calendar
            fullscreen={false}
            dateCellRender={dateCellRender}
            onSelect={handleSelectDate}
            ...
            <ul className="mt-3 space-y-2">
              {Array.from({ length: 7 }, (_, index) => {
                ...
                return (
                  <li
                    key={date}
                    className="flex flex-col gap-1 rounded border p-2 text-sm sm:flex-row sm:items-center sm:justify-between sm:text-base"
                  >
                    <span>{date}</span>
          ...
          <div className="rounded-lg bg-white p-4 shadow-sm">
            ...
            <ul className="mt-3 space-y-2">
              {unavailableDates.length > 0 ? (
                ...
                <li className="rounded border p-2 text-sm text-gray-600 sm:text-base">
